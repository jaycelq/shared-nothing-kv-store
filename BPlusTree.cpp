...
void BPlusTree::leafNode::insert(int k, string v)
{
    int i = 0;
    for(i = keyNum-1; i >= 0 && key[i] > k; i--) {
        key[i+1] = key[i];
...
bool BPlusTree::get(Node* node, int key, string & v)
{
...
bool BPlusTree::get(int key, string& result)
{
    return get(root, key, result);
}

void BPlusTree::insertNode(Node* node, int key, string v, Node *p, stack<Node*>* parent)
{
    Node* newNode;
    int newKey;
...
void BPlusTree::put(int key, string value)
{
    stack<Node *> parent;
    Node *n = root;
    if(root == NULL) {
